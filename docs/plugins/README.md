...
For Codex, the first supported executable path is project-level
`.codex/hooks.json` generated by:

```bash
nanomem install-codex-hooks --project-dir .
```

The repo-local Codex plugin remains useful for skills and packaging, but
plugin-bundled hooks must be validated separately in the target Codex CLI.
Local interactive Codex executed project hooks after hook trust; local
`codex exec` did not execute user, project, or plugin hooks in the checked
environment.
...
- `codex-installation.md`: Codex integration installation, runtime flow, validation,
  and troubleshooting.
