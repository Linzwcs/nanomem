...
main() {
  ...
  echo ""
  echo "This script does not install or trust Codex plugins."
  echo "Precondition: install nanomem-codex from this repo's marketplace and trust its hooks."
  echo ""
  ...
  PATH="$BIN_DIR:$PATH" \
    ...
    codex exec --enable plugin_hooks --json "$PROMPT" | tee "$CODEX_LOG"
